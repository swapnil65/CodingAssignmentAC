...
public class TestProductResource {
	...
	@Test
	public void testFindOne_ProductFound_ShouldReturnFoundProductEntry()
			throws Exception {
		when(productServiceMock.getProduct("2")).thenReturn(
				mockProducts.getSingleProduct());
		mockMvc.perform(
				get("/products/{productId}", "2").accept(
	...
	@Test
	public void testGetAllProducts_ProductsFound_ShouldReturnFoundProductEntries()
			throws Exception {

		when(productServiceMock.listProducts()).thenReturn(
				mockProducts.getAllProducts());

		mockMvc.perform(
				get("/products").contentType(MediaType.APPLICATION_JSON_UTF8))
				.andExpect(status().isOk())
				.andExpect(
						content().contentType(MediaType.APPLICATION_JSON_UTF8))
				...
				.andExpect(
						jsonPath("$[2].description", is("Floral Swing Skirt")))
				...
				.andExpect(
						jsonPath("$[4].description", is("True Skinny Jeans")));
		verify(productServiceMock, times(1)).listProducts();
		verifyNoMoreInteractions(productServiceMock);
	}

	@Test
	public void testGetProductsMoreThan30_ProductsFound_ShouldReturnFoundProductsEntries()
			throws Exception {
		when(productServiceMock.getProductsWithPriceMoreThan30()).thenReturn(
				mockProducts.getProductsMoreThan30());
		mockMvc.perform(
				get("/products/search/priceMoreThanThirty").contentType(
						MediaType.APPLICATION_JSON_UTF8))
				.andExpect(status().isOk())
				.andExpect(
						content().contentType(MediaType.APPLICATION_JSON_UTF8))
				...
				.andExpect(
						jsonPath("$[1].description", is("Floral Swing Skirt")))
				.andExpect(jsonPath("$[2].upc", is("1258793283")))
				.andExpect(
						jsonPath("$[2].description", is("True Skinny Jeans")));
