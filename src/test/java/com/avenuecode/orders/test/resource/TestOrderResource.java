...
public class TestOrderResource {
	...
	@Test
	public void testGetSingleOrder_OrderFound_ShouldReturnFoundOrderEntry()
			throws Exception {
		when(orderServiceMock.getOrder("3")).thenReturn(
				mockOrders.getSingleOrder());
		mockMvc.perform(
				get("/orders/{orderId}", "3").accept(
	...
	@Test
	public void testGetOrderByStatus_OrdersFound_ShouldReturnFoundOrdersEntries()
			throws Exception {
		when(orderServiceMock.searchOrders("shipped")).thenReturn(
				mockOrders.getOrderByStatus());
		mockMvc.perform(
				get("/orders/search/{searchCriteria}", "shipped").contentType(
						MediaType.APPLICATION_JSON_UTF8))
				.andExpect(status().isOk())
				.andExpect(
						content().contentType(MediaType.APPLICATION_JSON_UTF8))
	...
	@Test
	public void testGetOrderByDiscount_OrdersFound_ShouldReturnFoundOrdersEntries()
			throws Exception {
		when(orderServiceMock.searchOrders("discountApplied")).thenReturn(
				mockOrders.getOrderByDiscount());
		mockMvc.perform(
				get("/orders/search/{searchCriteria}", "discountApplied")
	...
	@Test
	public void testGetOrderMoreThanTwoProducts_OrdersFound_ShouldReturnFoundOrdersEntries()
			throws Exception {
		when(orderServiceMock.searchOrders("moreThanTwoProducts")).thenReturn(
				mockOrders.getOrderMoreThanTwoProducts());
		mockMvc.perform(
				get("/orders/search/{searchCriteria}", "moreThanTwoProducts")
						.contentType(MediaType.APPLICATION_JSON_UTF8))
				.andExpect(status().isOk())
				.andExpect(
						content().contentType(MediaType.APPLICATION_JSON_UTF8))
	...
	@Test
	public void testGetAllOrders_OrdersFound_ShouldReturnFoundOrderEntries()
			throws Exception {

		when(orderServiceMock.listOrders()).thenReturn(
				mockOrders.getAllOrders());

		mockMvc.perform(
				get("/orders").contentType(MediaType.APPLICATION_JSON_UTF8))
				.andExpect(status().isOk())
				.andExpect(
						content().contentType(MediaType.APPLICATION_JSON_UTF8))
